...
class mainRoot():
    ...
    def mainRootConfig(self, root: Tk):
        ...
        self.root.iconbitmap(getcwd()+"/MyMatcher/application/codest.ico")
        self.root.title(self.name)
        self.build()

    def build(self):
        self.button_A1 = Button(self.root, text="MyConnect", width=11, borderwidth=0,
                                border=0, bg="light gray", command=self.connector.build)
        self.button_A2 = Button(self.root, text="MyShare", width=11,
                                borderwidth=0, border=0, bg="light gray", command=self.filer.build)
        self.place()
